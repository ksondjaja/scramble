document.addEventListener('DOMContentLoaded', function(){
    ...
    for(var i=0; i<n; i++){
        ...
        cards[i].style.left = (cwidth+20)*(i%10) + 20 + 'px';
        fill.push(99);
        correct.push(i);
    }
    ...
    for(var i=0; i<n; i++){
        cards[i].style.top = lastbox.bottom + (cheight+20)*Math.floor(i/10) + 20 + 'px';
    }
    ...
    plays = document.querySelectorAll('.playvid')

    plays.forEach(
        function(play){
            play.onmouseover = function(){
                play.style.color = '#9e5ccd';
                play.style.borderColor = '#9e5ccd';
            }
            play.onmouseout = function(){
                play.style.color = 'cadetblue';
                play.style.borderColor = 'cadetblue';
            }
            play.onmousedown = function(event){
                event.stopPropagation();
                document.querySelector('.player').innerHTML = play.dataset.seconds;
            }
        }
    )
});